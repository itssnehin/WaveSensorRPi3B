    ...
    while(loop):
        ...
        if (final > (highVal + tolerance)):
            ...
            if (counter%groupingNum == 0):
                avgTime.append(round(temp/groupingNum, 4)) # Period of waves
                avgFreq.append(round(groupingNum/temp, 4)) # Frquency aray
                temp = 0        # reset temp
...
def dataTransfer(conn):
    ...
    while True:
        ...
        if command == '1':
            ...
            replyFreq = " ".join(str(x) for x in avgFreq) # Send array of speed in string form
            replyTime = " ".join(str(x) for x in log)
            conn.sendall(str.encode("Frequency values for 10 consecutive pulses each: \n"))
            conn.sendall(str.encode(replyFreq + " \n"))
            conn.sendall(str.encode("Time log: \n"))
            conn.sendall(str.encode(replyTime + "\n \n"))
        elif command == '3':
            reply = start()
        elif command == '4':
            reply = stop()
